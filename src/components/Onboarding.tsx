...
export default function Onboarding({ currentUser, onCompleteTeacher, onCompleteStudent }: OnboardingProps) {
  ...
  const handleTeacherSubmit = async () => {
    ...
    try {
      ...
      // Check if there is a prepaid plan from the landing page check-outs
      let promoSubscription: any = null;
      try {
        const localPrepaid = localStorage.getItem('flodech_prepaid_subscription');
        if (localPrepaid) {
          promoSubscription = JSON.parse(localPrepaid);
        }
      } catch (err) {
        console.warn('Could not read local prepaid records:', err);
      }
      ...
      const freshAcademy: Academy = {
        ...
        theme: 'orange',
        createdAt: new Date(),
        subscriptionStatus: promoSubscription ? 'active' : 'created',
        subscriptionPlan: promoSubscription ? 'Flodech Pro' : 'Disabled',
        subscriptionOrderId: promoSubscription?.orderId || '',
        subscriptionPaymentId: promoSubscription?.paymentId || '',
        subscriptionAmount: promoSubscription ? 499 : 0,
        subscriptionDate: promoSubscription ? new Date().toISOString() : null
      };

      if (promoSubscription) {
        try {
          localStorage.removeItem('flodech_prepaid_subscription');
        } catch (_) {}
      }
