...
export default function StudentDashboard({ studentId, studentEmail, onLogout }: StudentDashboardProps) {
  const [activeTab, setActiveTab] = useState<'profile' | 'notices' | 'schedule' | 'attendance' | 'fees' | 'portfolio' | 'chat' | 'homeworks'>('profile');
      ...
      try {
        ...
        if (sSnap.exists()) {
          ...
          if (aSnap.exists()) {
            ...
            setAcademyType(aData.academyType || '');
          }
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const studentBatches = student.batch 
      ? student.batch.split(',').map((b) => b.trim()).filter(Boolean) 
      : [];

    const schedulesQuery = studentBatches.length > 0 
      ? query(
          collection(db, 'schedules'),
          where('academyId', '==', student.academyId),
          where('batchName', 'in', studentBatches)
        )
      : query(
          collection(db, 'schedules'),
          where('academyId', '==', student.academyId),
          where('batchName', '==', 'DummyNonExistentBatchToPreventCrash')
        );
    ...
    const attendanceQuery = studentBatches.length > 0
      ? query(
          collection(db, 'attendance'),
          where('academyId', '==', student.academyId),
          where('batch', 'in', studentBatches)
        )
      : query(
          collection(db, 'attendance'),
          where('academyId', '==', student.academyId),
          where('batch', '==', 'DummyNonExistentBatchToPreventCrash')
        );
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row font-sans text-slate-800">
      ...
      <aside className="w-full md:w-64 bg-slate-900 text-white flex flex-col shrink-0">
        ...
        <nav className={`${isMobileMenuOpen ? 'flex' : 'hidden'} md:flex flex-col flex-grow p-4 space-y-1.5 overflow-y-auto`}>
          ...
          <button
            id="tab-homeworks-btn"
            onClick={() => {
              setActiveTab('homeworks');
              setIsMobileMenuOpen(false);
            }}
            className={`flex items-center px-4 py-3 rounded-xl text-sm font-semibold gap-3 transition-all ${activeTab === 'homeworks' ? 'bg-orange-500 text-white shadow-md' : 'text-slate-400 hover:text-white hover:bg-slate-800/45'}`}
          >
            <ClipboardList className="w-4 h-4" /> Homeworks
          </button>
      ...
      <main className="flex-grow p-6 sm:p-10 overflow-y-auto max-w-5xl mx-auto w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-1 bg-white p-6 border border-slate-200/60 rounded-3xl text-center shadow-lg shadow-slate-100">
              ...
              <div className="space-y-3.5 text-left text-xs">
                ...
                {/* Modular custom categorization rows */}
                {academyType === 'Coaching / Tuition' && student.academicLevel && (
                  <div className="flex justify-between pt-2 border-t border-slate-50">
                    <span className="text-slate-400 font-semibold">Academic Level:</span>
                    <span className="font-bold text-indigo-600">{student.academicLevel}</span>
                  </div>
                )}

                {academyType === 'Dance Academy' && (
                  <>
                    {student.danceStyle && (
                      <div className="flex justify-between pt-2 border-t border-slate-50">
                        <span className="text-slate-400 font-semibold">Dance Specialty:</span>
                        <span className="font-bold text-rose-600">{student.danceStyle}</span>
                      </div>
                    )}
                    {student.skillLevel && (
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-semibold">Dancer Rank:</span>
                        <span className="font-bold text-emerald-600">{student.skillLevel}</span>
                      </div>
                    )}
                  </>
                )}

                {academyType === 'Music Academy' && (
                  <>
                    {student.instrumentCategory && (
                      <div className="flex justify-between pt-2 border-t border-slate-50">
                        <span className="text-slate-400 font-semibold">Instrument:</span>
                        <span className="font-bold text-cyan-600">{student.instrumentCategory}</span>
                      </div>
                    )}
                    {student.skillLevel && (
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-semibold">Skill Level / Grade:</span>
                        <span className="font-bold text-amber-600">{student.skillLevel}</span>
                      </div>
                    )}
                  </>
                )}
            ...
            <div className="md:col-span-2 bg-white p-8 border border-slate-200/60 rounded-3xl shadow-lg shadow-slate-100 space-y-6">
              ...
              <div className="bg-slate-950 p-6 rounded-2xl text-white">
                <h4 className="text-xs font-bold text-orange-500 uppercase mb-2 flex items-center gap-1.5"><Building className="w-3.5 h-3.5" /> Academy Info</h4>
                <p className="text-xs text-slate-300 leading-relaxed font-sans font-medium">To update your contact profile, please contact your teacher or the academy administrator directly.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'fees' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2 space-y-6">
              <h3 className="font-bold text-base text-slate-900 uppercase">Fees & Invoices</h3>
        ...
        {activeTab === 'homeworks' && student && (
          <StudentHomeworks student={student} />
        )}
      </main>
    </div>
  );
