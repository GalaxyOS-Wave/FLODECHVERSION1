...
export default function PublicAcademyProfile({ academyId, onBackToPortal }: PublicAcademyProfileProps) {
  ...
  const themeBg = 'bg-orange-500';
  const themeBgHover = 'hover:bg-orange-600';
  const themeText = 'text-orange-600';
  const themeTextLight = 'text-orange-500';
  const themeBgLight = 'bg-orange-50';
  const themeBorder = 'border-orange-200';
  const themeBorderHover = 'hover:border-orange-200';
  const themeHoverText = 'hover:text-orange-500';
  const themeSelection = 'selection:bg-orange-500';
  const themeSpinner = 'border-orange-500';
  const themeRadial = 'from-orange-500/10';
  const themeBorderColor = 'border-orange-100';
  const themeFill = 'fill-orange-500';

  return (
    <div className={`min-h-screen bg-slate-50/50 flex flex-col ${themeSelection} selection:text-white relative font-sans leading-normal`}>
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`px-4 py-2 ${themeBg} ${themeBgHover} active:scale-95 text-white text-xs font-black uppercase tracking-widest rounded-xl shadow transition duration-150 cursor-pointer`}
            >
              Request Admission
            </button>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start text-center md:text-left gap-6 md:gap-8">
            ...
            <div className="flex-grow">
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2.5 mb-1.5">
                ...
                <span className={`${themeBgLight} ${themeText} font-black text-[9px] uppercase tracking-wider px-2.5 py-0.5 rounded-full flex items-center gap-1 border ${themeBorder}/50`}>
                  <ShieldCheck className={`w-3 h-3 ${themeFill} text-white`} />
                  Flodech Verified
                </span>
              </div>
      ...
      <div className="flex-grow max-w-4xl mx-auto w-full px-6 py-12 space-y-12">
        ...
        {(infoAbout || infoMission || infoStyle || infoWhy) && (
          <div className="bg-white p-8 sm:p-10 border border-slate-150 rounded-3xl shadow-sm space-y-6">
            <h2 className="text-2xl font-black text-slate-950 tracking-tight flex items-center gap-2 border-b border-slate-100 pb-3">
              <Building2 className={`w-5.5 h-5.5 ${themeTextLight}`} /> Academy Overview
            </h2>
        ...
        <div id="courses-offered-section" className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <span className={`text-[10px] uppercase font-bold tracking-widest ${themeTextLight}`}>Admissions Open</span>
          ...
          {courses.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {courses.map((course) => (
                <div 
                  key={course.id} 
                  className={`bg-white border border-slate-150 rounded-3xl p-6 flex flex-col justify-between ${themeBorderHover} transition duration-155 shadow-xs`}
                >
                  <div>
                  ...
                  {course.fees && (
                    <div className="border-t border-slate-100 pt-3 flex justify-between items-center bg-slate-50/50 -mx-6 -mb-6 px-6 py-3 rounded-b-3xl mt-4">
                      ...
                      <span className={`text-sm font-black ${themeText} font-mono`}>{course.fees}</span>
                    </div>
                  )}
                </div>
              ))}
        ...
        {settings.showTeachers && (
          <div className="space-y-6">
            <div>
              <span className={`text-[10px] uppercase font-bold tracking-widest ${themeTextLight}`}>Expert Mentors</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {teachers.map((teacher) => (
                  <div key={teacher.id} className="bg-white border border-slate-200/60 rounded-3xl p-5 text-center transition duration-150 hover:-translate-y-1 hover:shadow-md">
                    ...
                    <span className={`${themeTextLight} font-extrabold text-[10px] uppercase tracking-wider block mb-2`}>{teacher.role}</span>
        ...
        {settings.showPortfolios && (
          <div className="space-y-6">
            <div>
              <span className={`text-[10px] uppercase font-bold tracking-widest ${themeTextLight}`}>Outstanding Learners</span>
                ...
                {publicPortfolios.map(({ student, portfolio }) => (
                  <div key={student.id} className="bg-white border border-slate-200/60 rounded-3xl p-5 flex flex-col justify-between hover:border-slate-300 transition shadow-xs">
                    ...
                    <div className="border-t border-slate-100 pt-3 flex justify-between items-center text-[11px]">
                      <span className={`${themeBgLight} ${themeText} px-2 py-0.5 rounded font-black uppercase text-[9px] tracking-wide font-mono`}>
                        ...
                        onClick={() => {
                          ...
                          try {
                            ...
                            try {
                              ...
                              console.warn("Could not dispatch popstate event: ", err);
                            }
                          }
                        }}
                        className={`text-slate-900 ${themeHoverText} font-black flex items-center gap-0.5 uppercase tracking-wide text-[10px]`}
                      >
        ...
        {settings.showGallery && (
          <div className="space-y-6">
            <div>
              <span className={`text-[10px] uppercase font-bold tracking-widest ${themeTextLight}`}>Explore Classes</span>
        ...
        {settings.showEvents && (
          <div className="space-y-6">
            <div>
              <span className={`text-[10px] uppercase font-bold tracking-widest ${themeTextLight}`}>Stay Updated</span>
                ...
                {events.map((event) => (
                  <div key={event.id} className="bg-white border border-slate-150 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-start gap-4">
                      <div className={`${themeBgLight} ${themeTextLight} w-11 h-11 rounded-xl flex flex-col items-center justify-center shrink-0 border ${themeBorderColor} font-mono`}>
        ...
        {settings.showTestimonials && (
          <div className="space-y-6">
            <div>
              <span className={`text-[10px] uppercase font-bold tracking-widest ${themeTextLight}`}>Our Commitee Speaks</span>
        ...
        <div className="bg-slate-950 text-white p-8 sm:p-12 rounded-3xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
          <div className={`absolute inset-0 bg-radial-gradient ${themeRadial} via-transparent to-transparent opacity-60 pointer-events-none`} />
          ...
          <div className="space-y-2 text-center md:text-left z-10 relative">
            <h3 className="text-2xl sm:text-3xl font-black tracking-tight leading-none text-white flex items-center justify-center md:justify-start gap-2">
              <Sparkles className={`w-6 h-6 ${themeTextLight} animate-pulse`} /> Register with Flodech
            </h3>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto shrink-0 z-10">
            ...
            <button
              onClick={() => setShowAdmissionModal(true)}
              className={`px-6 py-3 ${themeBg} text-white font-black text-xs uppercase tracking-widest rounded-xl ${themeBgHover} active:scale-95 transition shadow-md cursor-pointer text-center`}
            >
              Request Admission
            </button>
          </div>
            ...
            >
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-lg ${themeBg} text-white flex items-center justify-center font-bold text-base shadow-inner`}>
                    A
                  </div>
                  <div>
              ...
              <div className="p-6 overflow-y-auto space-y-4">
                {admissionSuccess ? (
                  <center className="py-8 space-y-3">
                    ...
                    <button
                      onClick={() => { setShowAdmissionModal(false); setAdmissionSuccess(false); }}
                      className={`px-4 py-2 bg-slate-900 ${themeBgHover} text-white font-extrabold text-xs uppercase tracking-wider rounded-xl cursor-pointer transition shadow`}
                    >
                      Dismiss View
                    </button>
                  </center>
                  ...
                  <form onSubmit={handleAdmissionSubmit} className="space-y-4">
                    ...
                    <button
                      ...
                      className={`w-full ${themeBg} text-white py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest ${themeBgHover} cursor-pointer shadow flex items-center justify-center gap-1.5 transition`}
                    >
                ...
                {inquirySuccess ? (
                  ...
                  <form onSubmit={handleInquirySubmit} className="space-y-4">
                    ...
                    <button
                      ...
                      className={`w-full bg-slate-900 text-white py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest ${themeBgHover} cursor-pointer shadow flex items-center justify-center gap-1.5 transition`}
                    >
